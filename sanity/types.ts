...
// Query: *[_type == "startup" && defined(slug.current) && !defined($search) || title match $search || category match $search || author->name match $search] | order(_createdAt desc) {  _id,  title,  slug,  _createdAt,  author -> {    _id, name, image, bio  },  views,  description,  category,  image,  "voteCount": count(votes),  "hasVoted": defined($userId) && $userId in votes[]._ref,}
...
// Query: *[_type == "startup" && _id == $id][0]{  _id,  title,  slug,  _createdAt,  author -> {    _id, name, username, image, bio  },  views,  description,  category,  image,  pitch,  "voteCount": count(votes),  "hasVoted": defined($userId) && $userId in votes[]._ref,}
...
// Variable: STARTUP_SIMILAR_QUERY
// Query: *[_type == "startup" && _id != $id && defined(category) && category == $category] | order(views desc, _createdAt desc) [0...4] {  _id,  title,  slug,  _createdAt,  author -> {    _id, name, image, bio  },  views,  description,  category,  image,  "voteCount": count(votes),  "hasVoted": defined($userId) && $userId in votes[]._ref,}
export type STARTUP_SIMILAR_QUERYResult = Array<{
  _id: string;
  title: string | null;
  slug: Slug | null;
  _createdAt: string;
...
// Variable: STARTUP_VOTES_QUERY
// Query: *[_type == "startup" && _id == $id][0]{        _id, "voterIds": votes[]._ref    }
export type STARTUP_VOTES_QUERYResult = {
  _id: string;
...
// Query: *[_type == "author" && email == $email][0]{    _id,    id,    name,    username,    email,    image,    bio,    password}
...
// Query: *[_type == "startup" && author._ref == $id] | order(_createdAt desc) {  _id,  title,  slug,  _createdAt,  author -> {    _id, name, image, bio  },  views,  description,  category,  image,  "voteCount": count(votes),  "hasVoted": defined($userId) && $userId in votes[]._ref,}
